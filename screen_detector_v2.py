...
def run_detector():
    ...
    global distraction_count, total_focus_time, focus_start, distracted_start, alarm_playing
    ...
    while True:
        ...
        cv2.putText(frame, f"Distractions: {distraction_count}",
                    (20, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2)

        cv2.putText(frame, f"Focus Time: {int(total_focus_time)} sec",
                    (20, 180), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
...
btn1 = tk.Button(root, text="Start Tracking", command=start_app, height=2, width=20)
btn1.pack(pady=10)

btn2 = tk.Button(root, text="Reset Stats", command=reset_stats, height=2, width=20)
btn2.pack(pady=10)

btn3 = tk.Button(root, text="Exit", command=root.destroy, height=2, width=20)
btn3.pack(pady=10)
